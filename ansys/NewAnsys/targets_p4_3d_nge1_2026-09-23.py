...
PKG = os.path.normpath(os.path.join(HERE, "..", "..", "plate_solver",
                                    "piezo_solver.py"))
OUT = os.path.join(HERE, "targets_p4_3d_nge1_2026-09-23.json")
...
def roots_for(ps, den, log=print):
    ...
    for n in range(0, NMAX + 1):
        ...
        for i in range(700):
            ...
            lo, hi = el * (1 - 2e-3), el * (1 + 2e-3)
            olo, ohi = el * (1 - 1e-3), el * (1 + 1.5e-2)
            nf_sc = flips(lambda w: s40.coupled_det(w, n), lo, hi)
            nf_oc = flips(lambda w: s40.oc_ff_det(w, n), olo, ohi, 32)
...
def main(argv):
    ...
    data["meta"].update(dict(
        written="2026-09-23", solver=os.path.relpath(PKG, HERE),

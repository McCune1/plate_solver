#!/usr/bin/env python3
"""Build Liu 2002 solid-disk SC FE decks (Paper 4 lever 11).
...
VGLUE — identical LESIZE pairing to build_p4_3d_nge1. UX=0 on r=RI_MESH is
the 3-D regularity stand-in for the collapsed axis. No inner clamp/SS.
...
def stem(bc):
    return "ansys_p4_liu_disk_sc_%s_%s" % (bc, DATE)
...
def build(bc):
    ...
    # Inner cylindrical surface (regularity UX=0): nt x nz face
    ...
    a("! Built by build_liu_disk_sc_decks_%s.py. Do not hand-edit; rebuild." % DATE)
    ...
    a("! RI_MESH=%.1f mm (Liu RI=0; hole = %.2f%% of RO). Inner arcs=NDIV_TH," % (
        RI, 100.0 * RI / RO))
    a("! radials=NDIV_R, 28 lines. UX=0 on r=RI_MESH = axis regularity stand-in.")
    ...
    a("D,ALL,UY,0")
    a("ALLSEL,ALL")
    a("! inner regularity (Liu axis stand-in): UX=0 on r=RI_MESH; no clamp")
    a("CSYS,1")
    ...
    a("  *MSG,FATAL")
    a("Inner-hole regularity node count is wrong.")
    a("*ENDIF")
    a("D,ALL,UX,0")
    a("ALLSEL,ALL")

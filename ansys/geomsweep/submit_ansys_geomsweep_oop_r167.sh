#!/bin/bash
#SBATCH --job-name=geomsweep_oop_r167
#SBATCH --output=geomsweep_oop_r167_%j.out
#SBATCH --error=geomsweep_oop_r167_%j.err
#SBATCH --nodes=1
#SBATCH --ntasks=1
...
# ansys_geomsweep_oop_r167.inp (cd there, then `sbatch submit_ansys_geomsweep_oop_r167.sh`).
module load ansys/2024R2 2>/dev/null || module load ansys 2>/dev/null

echo "Job $SLURM_JOB_ID on $(hostname)"
echo "Start: $(date)"

mapdl -b -smp -np ${SLURM_CPUS_PER_TASK:-8} -i ansys_geomsweep_oop_r167.inp -o ansys_geomsweep_oop_r167_out.txt

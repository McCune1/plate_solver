#!/bin/bash
#SBATCH --job-name=ff_res_screen_geomsweep_s10v2_r150
#SBATCH --output=ff_res_screen_geomsweep_s10v2_r150_%j.out
#SBATCH --error=ff_res_screen_geomsweep_s10v2_r150_%j.err
#SBATCH --nodes=1
...
# Runs probe_ff_residual_screen_geomsweep_s10v2.py for RATIO_TAG=r150
# (r0/2b=1.5). Re-screens the FULL cut-off-adjacent candidate population
...
# submit_ff_residual_screen_s10v2_growth_r150.sh, which only screened the
...
# figures_ff_r150_s10v2.zip manifest) found exactly 53 targets (29 OOP +
# 24 IP) -- identical in count to the pre-s10 script's own r150 total,
...
# probes' own estimate) = ~2.2 CPU-hours, parallelized 1 point/worker
...
# `sbatch submit_ff_residual_screen_geomsweep_s10v2_r150.sh`) -- the
# script reads figures_ff_r150_s10v2/freefree_manifest.json as a
...
if [ ! -f figures_ff_r150_s10v2/freefree_manifest.json ]; then
  echo "FATAL: figures_ff_r150_s10v2/freefree_manifest.json not found. If"
  echo "figures_ff_r150_s10v2.zip is present instead, unzip it first:"
  echo "  mkdir -p figures_ff_r150_s10v2 && unzip -o -j figures_ff_r150_s10v2.zip freefree_manifest.json -d figures_ff_r150_s10v2"
  exit 2
fi

#!/bin/bash
#SBATCH --job-name=ff_residual_screen_r150
#SBATCH --output=ff_residual_screen_r150_%j.out
#SBATCH --error=ff_residual_screen_r150_%j.err
#SBATCH --nodes=1
...
# Runs probe_ff_residual_screen_geomsweep.py for RATIO_TAG=r150 (53
...
#
# Cost estimate: 53 points x ~150s/point serial (1 full_search + 1
# residual-screen build + 1 cheap sigma_min reuse, dps=40, mirroring
# probe_ff_oop_spurious_table.py's own per-point cost) = ~2.2 CPU-hours,
# parallelized 1 point/worker across 53 workers -- expect a few minutes
...
# submit_ff_residual_screen_r150.sh`).

#!/bin/bash
#SBATCH --job-name=ip_qin_mac
#SBATCH --output=probe_ip_qin_pair_mac_v1_%j.out
#SBATCH --error=probe_ip_qin_pair_mac_v1_%j.err
#SBATCH --nodes=1
#SBATCH --ntasks=1
...
# MAC on Qin 4.4293/4.4754 isolates (1.14274 and 1.14758).
# Same dump as the 1115 job. Submit AFTER the ANSYS fullblock:
#   sbatch --dependency=afterok:$ANSYS_JOB submit_ip_qin_pair_mac_v1.sh
#
# Copy onto Mill (package root):
#   probe_ip_ffp1_close_pair_mac_v1.py   (same file as the 1115 job)
#   submit_ip_qin_pair_mac_v1.sh
# Do NOT copy plate_solver/. Do NOT bump SOLVER_VERSION.
# Cost: 2 gates + 2 Omegas ~= 20 min.

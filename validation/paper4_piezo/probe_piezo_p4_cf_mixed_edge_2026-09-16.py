...
def main():
    ...
    g_stiffen_cf = (cf_oc > cf_el) and (0.002 < drel_cf < 0.005)
    g_sc_cf = abs(drel_cf_sc) < 0.0005
    ...
    g_stiffen_fc = (fc_oc > fc_el) and (1e-6 < drel_fc < 0.01)
    g_sc_fc = abs(drel_fc_sc) < 0.0005
    ...
    out_path = os.path.join(os.path.dirname(__file__) or ".",
                            "piezo_p4_cf_mixed_edge_results.json")

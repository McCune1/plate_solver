...
# Anchors from LESSONS_LEARNED.md Sec 18.175 / lever list item 2.
SWEEP = [
    ...
    ("1/12", 1.0 / 12, 0.280, 0.00557, 2.13),  # Sec 18.231; duan 0.301, 0.00600, 1.98
    ("1/10", 1.0 / 10, None, None, None),
    ("1/8",  1.0 / 8,  0.405, 0.00805, 1.48),  # duan 0.450, 0.00893, 1.33
    ("1/6",  1.0 / 6,  None, None, None),
    ("1/5",  1.0 / 5,  0.601, 0.01191, 1.00),  # duan 0.700, 0.01385, 0.86
...
def main():
    ...
    for label, ratio, exp_split, exp_k2, exp_g2 in SWEEP:
        ...
        g2_001 = (1e-4 * w_oc / abs(dw_de)) / E31_TRUE * 100.0
        g2_01 = (1e-3 * w_oc / abs(dw_de)) / E31_TRUE * 100.0
    ...
    json_path = os.path.join(out_dir, "piezo_p4_h1_ratio_sweep_results.json")
    with open(json_path, "w") as f:
        json.dump(results, f, indent=2)
    csv_path = os.path.join(out_dir, "piezo_p4_h1_ratio_sweep.csv")
    ...
    try:
        ...
        fig.tight_layout()
        png = os.path.join(out_dir, "piezo_p4_h1_ratio_sweep.png")

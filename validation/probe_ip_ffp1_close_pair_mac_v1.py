# -*- coding: utf-8 -*-
"""
probe_ip_ffp1_close_pair_mac_v1.py -- DIAGNOSTIC ONLY.
No SOLVER_VERSION bump. No package detector change.

Scores the two isolated dips of ONE close pair at FF-P1 (r0/2b=1.5,
2Theta/pi=0.5, nu=0.30) on the production MAC bar (ip_mac_bar.py,
threshold 0.744, RULE_LOCALMIN_RESID, class-safe max).

PAIR=1115  Table 7 1114.9/1115.3 Hz pair (job 2428351 isolates)
PAIR=qin   Qin 4.4293/4.4754 pair (jobs 2421062/2421063 + 2421905)

Requires the nu=0.30 full-block dump from
submit_ansys_ip_ffp1_mac_fullblock.sh (prefix ip_mac_ffp1_nu030).
Do NOT use ip_mac_gs_r150_a050_* -- that prefix is the nu=0.35 Rank 14 dump.
...
-----------------------------------------------------------------------------
CP-0 GATE_FAIL
    SOLVER_VERSION, missing dumps, mac() identity, THETA, or either
    GATE_REAL / GATE_ART disagrees with the published FF-P1 MAC by >5e-4.
    VOID. Do not edit the paper.
CP-A TWO_SHAPES
    Both isolates are MAC-REAL and their best same-class FE modes differ.
    Two physical shapes. Production still lists one root; the footnote
    can say MAC sees two.
CP-B ONE_SHAPE
    Both isolates are MAC-REAL against the SAME best same-class FE mode.
    One physical shape, two frequencies (Qin/Table-7 pattern in reverse).
CP-C SPLIT
    One MAC-REAL, one MAC-ART. Assign the REAL-like member; leave the
    other as a nearby artifact. Table still not split by the detector.
CP-D BOTH_ART
    Neither isolate is MAC-REAL. Do not claim either is a physical
    partner of the FE pair. Leave the footnote as "MAC run; neither
    isolate is a physical shape at 0.744."
CP-E UNRELIABLE
    Gates pass but an isolate failed to reconstruct. Report; no table edit.
...
# FF-P1 calibration gates (job 2431872 / block-selection GATE)
GATES = [
    ("GATE_REAL", 0.401154, 1.0000, "q1"),
    ("GATE_ART", 1.454616, 0.0067, "q0"),
]
...
def load_adv():
    ...
    if path is None:
        ...
        raise SystemExit(2)
    spec = importlib.util.spec_from_file_location("adv_ffp1_cp", path)
...
def score_one(name, Om, adv, bar, solver, FEB, mode_hz, mode_cls, modes):
    ...
    fe_any = FEB[modes[0]]
    ux_s, uy_s, bad = adv.solver_field_at_fe(rp, tp, Ur, Uth, fe_any)
    keep = ~bad
    ...
    mac_same = sorted(((mac_by[m], m) for m in same), reverse=True)
    mac_best, best = mac_same[0]
    ...
    return dict(
        name=name, Om=Om, ok=True, pick=pick, want_cls=want_cls,
        mac_best=mac_best, best=best, call=call,
        rTyy=blocks[pick]["rTyy"], f_hz=f_hz(Om),
        mac_at_best_verb=verb,
    )
...
def main():
    ...
    if PAIR not in PAIRS:
        ...
        raise SystemExit(2)
    spec = PAIRS[PAIR]
    hdr(f"probe_ip_ffp1_close_pair_mac_v1  PAIR={PAIR}  {spec['label']}")
    ...
    dmac = abs(bar.mac(rng.normal(size=200), rng.normal(size=200))
               - adv.mac(rng.normal(size=200), rng.normal(size=200)))
    # identity on the same vectors:
    ...
    if freq_path is None:
        print(f"READING: CP-0 GATE_FAIL  (missing {PREFIX}_freqs.txt -- "
              f"run submit_ansys_ip_ffp1_mac_fullblock.sh first)", flush=True)
        raise SystemExit(2)
    ...
    print(f"  live parity SYMM={n_s} ANTI={n_a} AMBIGUOUS={n_u}  "
          f"(PHI={PHI_DEG}, fallback expected at 90 deg)", flush=True)
    ...
    gate_fail = []
    for gname, Om, mac_pub, blk_pub in GATES:
        hdr(f"{gname}  Omega={Om}  published MAC {mac_pub:.4f}")
        row = score_one(gname, Om, adv, bar, solver, FEB, mode_hz,
                        mode_cls, modes)
        if not row.get("ok"):
            gate_fail.append(f"{gname} {row.get('why')}")
            continue
        d = abs(row["mac_best"] - mac_pub)
        print(f"  GATE vs {mac_pub:.4f}: max-same={row['mac_best']:.4f} "
              f"|d|={d:.2e}", flush=True)
        if d > MAC_ATOL:
            gate_fail.append(f"{gname} MAC {row['mac_best']:.4f}!={mac_pub:.4f}")
        if blk_pub and row["pick"] != blk_pub:
            print(f"  note: pick {row['pick']} vs published {blk_pub} "
                  f"(not a void if MAC gate passes)", flush=True)
    ...
    for name, Om, note in spec["cands"]:
        ...
        row = score_one(name, Om, adv, bar, solver, FEB, mode_hz,
                        mode_cls, modes)
        row["note"] = note
        rows.append(row)
        if not row.get("ok"):
    ...
    isolates = [r for r in rows if r["name"].startswith("ISO") or
                (PAIR == "qin" and r["name"] in ("PROD", "ISO_B"))]
    # For Qin, PROD and ISO_B are the two dips. For 1115, ISO_A and ISO_B.
    if PAIR == "1115":
    ...
    if any(not r.get("ok") for r in pair_rows) or len(pair_rows) < 2:
        ...
        if n_real == 0:
            ...
            note = ("Neither isolate is MAC-REAL. Do not claim a physical "
                    "partner. Footnote: MAC run; both ARTIFACT-like at 0.744.")
        elif n_real == 1:
            letter = "CP-C SPLIT"
